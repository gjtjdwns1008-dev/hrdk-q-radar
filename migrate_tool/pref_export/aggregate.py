...
from .ledger_adapter import LedgerRecord

KST = timezone(timedelta(hours=9), name="KST")

# ── 대표 법령 선정 (임시 규칙) ─────────────────────────────────────────
# TODO(확인 3): 규격 v1.1 §1 '대표 법령 규칙' 원문으로 교체할 것.
#   아래는 자리표시 규칙(성격 우선순위 → 법령명 가나다)이며, Q-Page 검산은
#   top≤3·표시값만 보므로 형식은 통과하지만 '어떤 법령이 대표인가'는 v1.1이 정본.
TOP_LIMIT = 3
...
def build_snapshot(records: Iterable[LedgerRecord],
                   code_table: dict[str, str],
                   ts: datetime | None = None) -> PrefSnapshot:
    ts = ts or now_kst()
    rep = ExportReport()

    # 1) 원자화 레코드 수집 + 완전중복 제거
    ...
    for qual, law, raw_cat in sorted(triples):
        ...
        slot["counts"][jkey] += 1
        slot["laws"].append((law, catmap.to_display(raw_cat)))
    ...
    for qual, slot in by_qual.items():
        ...
        if not codemap.is_certification_code(code):
            ...
            continue
        items[code] = QualAgg(code=code, name=qual,
...
def _pick_top(laws: list[tuple[str, str]]) -> list[tuple[str, str]]:
    """대표 법령 최대 3건. TODO(확인 3): v1.1 §1 규칙으로 교체 — 아래는 임시."""
    prio = {c: i for i, c in enumerate(TOP_CAT_PRIORITY)}
    seen: set[str] = set()
    out: list[tuple[str, str]] = []
    for law, disp in sorted(laws, key=lambda x: (prio.get(x[1], 99), x[0])):
        if law in seen:
            continue
        seen.add(law)
        out.append((law, disp))
