...
■ 인증: GitHub Secrets → 환경변수(GCP_SERVICE_ACCOUNT_JSON, GOOGLE_SHEET_URL)
        → 공용 접속 부품(sheets_client, 읽기 전용). 이 파일은 .env를 절대 읽지 않는다.
...
# ── 자립 시동: 직접 실행돼도 조립 세트로 등록(그래야 아래 가드가 말할 수 있다) ──
...
from . import aggregate, codemap, json_writer, sheets_client, source_adapter, summary, validate
...
def main() -> int:
    ...
    text: str | None = None
    problems: list[str] = []
    # 기준연도는 실패 경로의 Summary에서도 쓰므로 try 밖에서 확정.
    # TODO(Phase 2): 게시 HTML의 명찰(qpage-master) year 자동 판독으로 교체.
    base_year = int(os.environ.get("QPAGE_BASE_YEAR", codemap.DEFAULT_BASE_YEAR))

    try:
        sa = os.environ.get("GCP_SERVICE_ACCOUNT_JSON", "")
        sheet_ref = os.environ.get("GOOGLE_SHEET_URL", "")
        if not sa or not sheet_ref:
            raise RuntimeError("시크릿 미주입: GCP_SERVICE_ACCOUNT_JSON / GOOGLE_SHEET_URL "
                               "(워크플로 스텝의 env 블록 확인)")
        ss = sheets_client.connect_readonly(sa, sheet_ref)   # 읽기 전용
        code_table, canonical = codemap.load_maps(args.codemap, year=base_year)
